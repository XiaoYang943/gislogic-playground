...
// 引入 iframe 链接
import srcdoc from '../../../public/html/srcdoc.html?raw'
import { PreviewProxy } from '../../core/PreviewProxy'
import { compileModulesForPreview } from '../../core/moduleCompiler'
import type { Props } from './Repl.vue'
import { injectKeyStore } from '../../type/types'
...
const initIframe = () => {
  ...
  sandbox.setAttribute(
    ...
    [
      ...
      'allow-modals',
      'allow-pointer-lock',
      'allow-popups',  // 　　允许弹出窗口（如window.open，target="_blank"）。
      'allow-same-origin',  // 允许 iframe 内容被视为与包含文档有相同的来源。
      'allow-scripts',  // 　　允许弹出窗口逃离沙箱：允许一个沙箱文件打开新窗口不强制使用沙盒。允许脚本执行，即允许iframe运行脚本（但不创建弹出窗口）。可用于禁用外部网站的JS
      'allow-top-navigation-by-user-activation'    // 　　允许 iframe 内容从包含文档导航（加载）内容。可用于禁用外部网站的JS跳转、target="_parent"、target="_top"等
    ].join(' ')
  )
}
/**
