...
import { compileFile } from '../core/transform'
import { atou, utoa } from '../utils/common/util'
import type {
  SFCAsyncStyleCompileOptions,
  SFCScriptCompileOptions,
  SFCTemplateCompileOptions,
} from 'vue/compiler-sfc'
import type { OutputModes } from '../type/types'
import type { editor } from 'monaco-editor-core'
import { type ImportMap, mergeImportMap, useVueImportMap } from '../core/import-map'

import welcomeSFCCode from '../components/template/DefaultSFC.vue?raw'
import CesiumSFCCode from '../components/template/CesiumSFC.vue?raw'
import OpenlayersSFCCode from '../components/template/OpenlayersSFC.vue?raw'
import newSFCCode from '../components/template/NewSFC.vue?raw'
...
export function useStore(
  {
    ...
    mainFile = ref('src/Repl.vue'),
    template = ref({
      welcomeSFC: welcomeSFCCode,
      newSFC: newSFCCode,
      CesiumSFC: CesiumSFCCode,
      OpenlayersSFC: OpenlayersSFCCode
    }),
  ...
  const setFiles: ReplStore['setFiles'] = async (newFiles, mainFile = store.mainFile,) => {
    ...
    if (!newFiles[mainFile]) {
      setFile(files, mainFile, template.value.welcomeSFC || welcomeSFCCode || CesiumSFCCode)
    }
  ...
  if (serializedState) {
    ...
    // setFile(files.value, mainFile.value, template.value.welcomeSFC || welcomeSFCCode || CesiumSFCCode,)
    setFile(files.value, mainFile.value, CesiumSFCCode) // 默认使用的SFC模板
